<nav class="nav box" id="nav">
    <svg class="nav__search">
        <use xlink:href="#icon-search" href="#icon-search"></use>
    </svg>
    <div class="nav__container box__container">
        <div class="nav__link-box --about">
            <div class="nav__link">О нас</div>
            <div class="nav__link-more">
                <a class="nav__link-more-item" href="<?php echo site_url('/about'); ?>">О компании</a>
                <a class="nav__link-more-item" href="<?php echo site_url('/pechat-knig'); ?>"> Печать книг</a>
                <a class="nav__link-more-item" href="<?php echo site_url('/liczenzii'); ?>"> Лицензии</a>
                <a class="nav__link-more-item" href="<?php echo site_url('/dostizheniya'); ?>"> Достижения </a>
                <a class="nav__link-more-item" href="<?php echo site_url('/reviews'); ?>"> Отзывы </a>
                <a class="nav__link-more-item" href="<?php echo site_url('/news'); ?>"> Новости </a>
                <a class="nav__link-more-item" href="<?php echo site_url('/smi-o-nas'); ?>"> СМИ о нас </a>
            </div>
        </div>
        <a class="nav__link-box" href="<?php echo site_url('/team'); ?>">
            <div class="nav__link">Наша команда</div>
        </a>
        <a class="nav__link-box" href="<?php echo site_url('/izdat-knigu'); ?>">
            <div class="nav__link">Издать книгу</div>
        </a>
        <a class="nav__link-box" href="<?php echo site_url('/portfolio'); ?>">
            <div class="nav__link">Портфолио</div>
        </a>
        <a class="nav__link-box" href="<?php echo site_url('/contacts'); ?>">
            <div class="nav__link">Контакты</div>
        </a>
    </div>
</nav>